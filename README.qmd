...
existing_software[, link_id := paste0("sec-", link_id)]
...
for (i in 1:nrow(existing_software)) {
    cat(
        sprintf(
            ...
            existing_software$link_id[i]
        ),
        file = "profiles.qmd", append = TRUE)
    cat(sprintf("%s\n\n", existing_software$Description[i]), file = "profiles.qmd", append = TRUE)
}

system("quarto render profiles.qmd --to gfm")
```